...
const FaqItem = ({item}) => {
  ...
  return (
    <div className='p-3 lg:p-5 rounded-[12px] border border-solid border-[#d9dce2] mb-5 cursor-pointer'>
        ...
        <h4 className='text-[14px] leading-7 lg:text-[22px] lg:leading-8 text-[#393939]'>{item.question}</h4>
        <div className={` ${isOpen && 'bg-[#0067ff] text-white border-none'}w-7 h-7 lg:w-8 lg:h-8 border border-solid border-[#141f21] rounded flex items-center justify-center`}>
          ...
          {isOpen && <div className='mt-4'>
            <p className='text-[14px] leading-6 lg:text-[16px] lg:leading-7 font-[400] text-[#4e545f]'>
                {item.content}
            </p>
            </div>}
    </div>
